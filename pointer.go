...
import (
	...
	"github.com/avila-r/constraints"
)
...
// Many takes a variadic argument of any type T and returns a slice of pointers
...
//
//	a := []int{1, 2, 3}
//	ptrs := pointer.Many(a...) // ptrs is []*int
func Many[T any](t ...T) []*T {
	r := []*T{}
	for _, v := range t {
		r = append(r, &v)
	}
...
func MapValues[K constraints.Comparable, T any](t map[K]T) map[K]*T {
	if t == nil {
		return nil
	}
